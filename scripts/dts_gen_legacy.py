...
#define LQ_MAX_HEALTH_DEVICES       {num_health_devices}
...
""".format(
            ...
            num_health_devices=counts['num_health_devices'],
...
def parse_uds_exposures(nodes):
    """
    Parse UDS protocol nodes and extract what should be exposed via DIDs.
    
    Returns a list of UDS exposure configs, each containing:
    - target_label: Label of the node to expose
    - target_node: Reference to the actual node
    - did_read: DID for reading (or None)
    - did_write: DID for writing (or None)
    - writable: Boolean flag
    - read_only: Boolean flag
    - security_level: Required security level (0-2)
    ...
    for uds_node in uds_nodes:
        # UDS node properties would contain child node definitions
        # Since our simple parser doesn't handle nested children well,
        # we'll look for specially named properties that define exposures
        
        # Alternative: Look for properties that reference other nodes
        for prop_name, prop_value in uds_node.properties.items():
            ...
            # e.g., expose_throttle_scale_read = <0xF200>
...
def generate_uds_handlers(nodes, output_path):
    """
    Generate UDS DID handler table and read/write functions.
    
    Parses UDS protocol nodes to determine what should be exposed,
    then generates type-safe accessor functions for each DID.
    """
    exposures = parse_uds_exposures(nodes)
    
    if not exposures:
        # No UDS exposures defined
        return
    
    # Validate: if exposures exist, must have UDS node
    uds_nodes = [n for n in nodes if n.compatible == 'lq,protocol-uds']
    if not uds_nodes:
        print("ERROR: UDS exposures found but no UDS stack defined!", file=sys.stderr)
        print(f"\nFound exposures for: {', '.join(e['target_label'] for e in exposures)}", file=sys.stderr)
 ...
 * Generated from devicetree by scripts/dts_gen.py
 * 
 * UDS DID handler table for accessing driver parameters
 */

#ifndef LQ_GENERATED_UDS_H_
...
""")
        ...
        for exp in exposures:
            ...
            if exp['did_read'] is not None:
                f.write(f"#define UDS_DID_{label_upper}_READ  0x{exp['did_read']:04X}\n")
            if exp['did_write'] is not None:
                f.write(f"#define UDS_DID_{label_upper}_WRITE 0x{exp['did_write']:04X}\n")
        
        f.write("\n/* UDS handler function */\n")
        f.write("int lq_generated_uds_handler(uint16_t did, bool is_write,\n")
        f.write("                             const uint8_t *in_data, size_t in_len,\n")
        f.write("                             uint8_t *out_data, size_t *out_len);\n")
        
        f.write("\n#ifdef __cplusplus\n")
        f.write("}\n")
        f.write("#endif\n\n")
        f.write("#endif /* LQ_GENERATED_UDS_H_ */\n")
    
    # Generate implementation file
    impl_path = output_path.parent / 'lq_generated_uds.c'
...
#include "lq_generated_uds.h"
#include "lq_generated.h"
#include <string.h>

/* External driver references (from generated code) */
...
""")
        ...
        for exp in exposures:
            ...
            # Read handler
            if exp['did_read'] is not None:
                f.write(f"        case UDS_DID_{label_upper}_READ:\n")
                f.write(f"            if (is_write) return -1;  /* Read-only DID */\n")
                
                # Generate type-specific read code
                if compatible in ['lq,scale', 'lq-scale']:
                    f.write(f"            /* Read scale configuration */\n")
                    f.write(f"            if (*out_len < 8) return -1;\n")
                    signal_id = target_node.signal_id or 0
                    f.write(f"            /* TODO: Add scale context lookup by signal ID {signal_id} */\n")
                    f.write(f"            /* For now, return signal value */\n")
                    f.write(f"            if ({signal_id} >= g_lq_engine.num_signals) return -1;\n")
                    f.write(f"            int32_t value = g_lq_engine.signals[{signal_id}].value;\n")
                    f.write(f"            out_data[0] = (value >> 0) & 0xFF;\n")
                    f.write(f"            out_data[1] = (value >> 8) & 0xFF;\n")
                    f.write(f"            out_data[2] = (value >> 16) & 0xFF;\n")
                    f.write(f"            out_data[3] = (value >> 24) & 0xFF;\n")
                    f.write(f"            *out_len = 4;\n")
                    f.write(f"            return 0;\n")
                
                elif compatible == 'lq,remap':
                    f.write(f"            /* Read remap configuration */\n")
                    f.write(f"            /* TODO: Implement remap read */\n")
                    f.write(f"            return -1;\n")
                
                elif compatible == 'lq,pid':
                    f.write(f"            /* Read PID parameters */\n")
                    f.write(f"            /* TODO: Implement PID read */\n")
                    f.write(f"            return -1;\n")
                
                elif compatible.startswith('lq,hw-'):
                    # Hardware input - return signal value
                    f.write(f"            /* Read hardware input signal value */\n")
                    signal_id = target_node.signal_id or 0
                    f.write(f"            if ({signal_id} >= g_lq_engine.num_signals) return -1;\n")
                    f.write(f"            if (*out_len < 4) return -1;\n")
                    f.write(f"            int32_t value = g_lq_engine.signals[{signal_id}].value;\n")
                    f.write(f"            out_data[0] = (value >> 0) & 0xFF;\n")
                    f.write(f"            out_data[1] = (value >> 8) & 0xFF;\n")
                    f.write(f"            out_data[2] = (value >> 16) & 0xFF;\n")
                    ...
                    f.write(f"            /* Unknown compatible: {compatible} */\n")
                    f.write(f"            return -1;\n")
            ...
            if exp['did_write'] is not None and exp['writable']:
                f.write(f"        case UDS_DID_{label_upper}_WRITE:\n")
                f.write(f"            if (!is_write) return -1;  /* Write-only DID */\n")
                f.write(f"            /* TODO: Implement write for {exp['target_label']} */\n")
                f.write(f"            /* Security level: {exp['security_level']} */\n")
                f.write(f"            return -1;  /* Not yet implemented */\n\n")
...
""")
    ...
    print(f"Generated {output_path} (UDS DID handlers)")
    print(f"Generated {impl_path} (UDS implementation)")
...
def main():
    ...
    # Generate UDS handlers if UDS protocol is used
    generate_uds_handlers(nodes, output_dir / 'lq_generated_uds.h')
