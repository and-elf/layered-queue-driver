...
/**
 * @brief Dispatch output events to hardware/protocol drivers
 * 
 * This function is generated by dts_gen.py based on the devicetree.
 * It contains a switch statement that routes output events to the
 * appropriate platform drivers (CAN, GPIO, PWM, etc.).
 * 
 * If no generated code is present (e.g., during library compilation),
 * a weak default implementation is provided that does nothing.
 */
void lq_generated_dispatch_outputs(void) __attribute__((weak));

/* Default weak implementation - does nothing if no generated code */
void lq_generated_dispatch_outputs(void)
{
    /* No-op: Will be overridden by generated code in application builds */
}
